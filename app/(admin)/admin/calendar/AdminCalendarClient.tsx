...
function buildMonthDays(monthDate: Date) {
  ...
  for (let index = leadingDays; index > 0; index--) {
    days.push({ date: new Date(firstDay.getFullYear(), firstDay.getMonth(), 1 - index), inMonth: false });
  }

  for (let day = 1; day <= lastDay.getDate(); day++) {
    days.push({ date: new Date(monthDate.getFullYear(), monthDate.getMonth(), day), inMonth: true });
  }

  while (days.length % 7 !== 0) {
    const nextDay = days.length - leadingDays - lastDay.getDate() + 1;
    days.push({ date: new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, nextDay), inMonth: false });
  }
    ...
    <section className="admin-calendar-shell">
              ...
              >
                ...
                <span className="admin-calendar-day-preview">
                  {dayBookings.slice(0, 2).map((booking) => (
                    <small key={booking.id}>{timeValue(booking.startTime)} {booking.clientName}</small>
                  ))}
                  {!dayBookings.length && dayBlocks.length > 0 && <small>Blockiert</small>}
      ...
      <aside className="admin-calendar-side admin-panel">
        <div className="admin-panel-head">
          ...
          <button className="admin-secondary-button" onClick={() => setBlockingDate(selectedDate)} type="button">
            Zeit blockieren
          </button>
          <AdminBookingCreator
        ...
        <div className="admin-calendar-events">
          {selectedBookings.map((booking) => (
            <button className="admin-calendar-event" key={booking.id} onClick={() => setEditingBooking(booking)} type="button">
          ...
          {selectedBlocks.map((block) => (
            <article className="admin-calendar-block" key={block.id}>
              <div>
                <span>{block.fullDay ? "Ganzer Tag" : `${timeValue(block.startTime)} - ${timeValue(block.endTime)}`}</span>
                <strong>{block.reason || "Blockiert"}</strong>
              </div>
              <form action={deleteBlockAction}>
                <input name="id" type="hidden" value={block.id} />
                <button className="admin-danger-button" type="submit">Entfernen</button>
              </form>
            </article>
          ))}

          {!selectedBookings.length && !selectedBlocks.length && <p className="admin-empty">Keine Termine oder Sperren fuer diesen Tag.</p>}
        </div>
      </aside>

      {blockingDate && (
        <div className="admin-modal-backdrop" role="dialog" aria-label="Zeit blockieren" aria-modal="true">
          <form
            action={createBlockAction}
            className="admin-modal admin-calendar-modal"
            onSubmit={() => setBlockingDate(null)}
          >
            <button className="admin-modal-close" onClick={() => setBlockingDate(null)} type="button">
              <X size={22} />
            </button>
            <div className="admin-panel-head">
              <div>
            ...
            <div className="admin-form-grid">
              ...
              <label>
                Notiz optional
                <input name="reason" placeholder="Ferien, privat, Werkstatt..." type="text" />
              </label>
            ...
            <button className="admin-submit-button" type="submit">Sperre speichern</button>
          </form>
        </div>
      )}

      {editingBooking && (
        <div className="admin-modal-backdrop" role="dialog" aria-label="Buchung bearbeiten" aria-modal="true">
          <div className="admin-modal admin-calendar-modal">
            <button className="admin-modal-close" onClick={() => setEditingBooking(null)} type="button">
              <X size={22} />
            </button>
            <div className="admin-panel-head">
              <div>
            ...
            <div className="admin-calendar-booking-summary">
              ...
              {editingBooking.addOns.length > 0 && <span>Extras: {editingBooking.addOns.join(", ")}</span>}
            </div>
            <form action={updateBookingAction} className="admin-form-grid" onSubmit={() => setEditingBooking(null)}>
              ...
              <button className="admin-submit-button" type="submit">Buchung speichern</button>
            </form>
            <form action={cancelBookingAction} className="admin-calendar-cancel-form" onSubmit={() => setEditingBooking(null)}>
              <input name="id" type="hidden" value={editingBooking.id} />
              <button className="admin-danger-button" type="submit">Buchung stornieren</button>
            </form>
          </div>
        </div>
      )}
